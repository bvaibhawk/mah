...
#     with open('diamond2.pkl', 'rb') as handle:
...
#     with col9:
#         pur_rap_dis = st.number_input("PUR RAP DIS")

#     test_df = pd.DataFrame({'SZ GR':[szgr], 'CERTCT':[certct], 'COLOR':[color_dict[color]], 'CLARITY':[clarity_dict[clarity]], 'CUT':[cut],
#                             'POLISH':[polish], 'SYMMETRY':[symmetry], 'FLUO':[fluo], 'PUR RAP DIS':[pur_rap_dis]})

#     result = model2.predict(test_df)[0]
#     st.text("\n")
...
#     with open('diamond3.pkl', 'rb') as handle:
...
#     with col10:
#         diff = st.number_input("Difference")

#     test_df = pd.DataFrame({'SZ GR':[szgr], 'CERTCT':[certct], 'COLOR':[color_dict[color]], 'CLARITY':[clarity_dict[clarity]], 'CUT':[cut],
#             'POLISH':[polish], 'SYMMETRY':[symmetry], 'FLUO':[fluo], 'PUR RAP DIS':[pur_rap_dis], 'Difference':[diff]})

#     result = model3.predict(test_df)[0]
#     st.text("\n")
...
#     if st.button("Calculate Price"):
#         if int(rap)!=0:
#             delta = (rap-result)/rap
#             if delta>0:
#                 st.markdown(f"<b>RAP Price is</b> <font size=5>₹ {int(rap)}</font>",unsafe_allow_html=True)
#                 st.markdown(f"<big><b>Our Price is</b> </big><font color='#daa522' size=6>₹ {int(result)}</font>&nbsp; &nbsp;<font color='green'><b>↓ {abs(int(delta*100))} %</b> discounted price than RAP</font>",unsafe_allow_html=True)
#             else:
#                 st.markdown(f"<b>RAP Price is</b> <font size=5>₹ {int(rap)}</font>",unsafe_allow_html=True)
#                 st.markdown(f"<big><b>Our Price is</b> </big><font color='#daa522' size=6>₹ {int(result)}</font>&nbsp; &nbsp;<font color='red'><b>↑ {abs(int(delta*100))} %</b> inflated price than RAP</font>",unsafe_allow_html=True)
#         else:
#             st.error("RAP Cannot be Zero!")

# def page5():

#     with open('diamond4.pkl', 'rb') as handle:
#         model = pickle.load(handle)
...
#     with col1:
#         size = st.selectbox("Size",options=['1.01-1.09', '1.5-1.69', '2.01-2.09'])

#     with col2:
#         color = st.selectbox("Color",options=['D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M'])

#     with col3:
#         clarity = st.selectbox("Clarity",options=['IF', 'VVS1', 'VVS2', 'VS1', 'VS2', 'SI1', 'SI2', 'I1'])\

#     with col4:
#         month = st.selectbox("Month",options=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct',
#        'Nov', 'Dec'])

#     with col5:
#         rapper = st.number_input("Rapper")

#     with col6:
#         rappri = st.number_input("Rappri")


#     test_df = pd.DataFrame({'Size':[size], 'Colour':[color], 'Clarity':[clarity], 'Month':[month], 'Rapper':[rapper], 'Rappri':[rappri]})
...
#     if st.button("Calculate Price"):
#         if int(rappri)!=0:
#             delta = (rappri-result)/rappri
#             if delta>0:
#                 st.markdown(f"<b>RAP Price is</b> <font size=5>₹ {int(rappri)}</font>",unsafe_allow_html=True)
#                 st.markdown(f"<big><b>Our Price is</b> </big><font color='#daa522' size=6>₹ {int(result)}</font>&nbsp; &nbsp;<font color='green'><b>↓ {abs(int(delta*100))} %</b> discounted price than RAP</font>",unsafe_allow_html=True)
#             else:
#                 st.markdown(f"<b>RAP Price is</b> <font size=5>₹ {int(rappri)}</font>",unsafe_allow_html=True)
#                 st.markdown(f"<big><b>Our Price is</b> </big><font color='#daa522' size=6>₹ {int(result)}</font>&nbsp; &nbsp;<font color='red'><b>↑ {abs(int(delta*100))} %</b> inflated price than RAP</font>",unsafe_allow_html=True)
#         else:
#             st.error("RAPPRI Cannot be Zero!")
