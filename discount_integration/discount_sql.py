...
def get_diameter_premium(shape, cut, poly, sym, fluo, size, diameter_min, diameter_max, color, clarity, sizeprec):
    ...
    if cut == 'EX' and poly == 'EX' and sym == 'EX':
        query_output = sqldf("select DISCOUNT from data_diameter where SHAPE = '" + shape
                             ...
                             + "' and SYM = '" + sym
                             + "' and FLUO = '" + fluo
                             + "' and SIZE_MIN <= " + str(sizeprec)
                             + " and SIZE_MAX >= " + str(sizeprec)
